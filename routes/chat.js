...
router.get('/messageList', auth, async (req, res) => {
    ...
    data.forEach(msg => {
        if (msg.seen == 0 && msg.sender !== req.userData.type){
            msg.seen = false;
        } else {
            msg.seen = true;
        }
...
router.get('/chat/:employee_id/:company_id/oldMessages', auth, async (req, res) => {
    try {
        ...
        const seen = await poolQuery("UPDATE messages SET seen=1 WHERE employee_id=? AND company_id=? AND sender!=?", [req.params.employee_id, req.params.company_id, req.userData.type])
        const seen2 = await poolQuery("UPDATE messagelist SET seen=1 WHERE employee_id=? AND company_id=? AND sender!=?", [req.params.employee_id, req.params.company_id, req.userData.type])
