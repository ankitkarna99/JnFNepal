...
router.get('/',async (req, res) => {
    ...
    var jobs = await poolQuery("SELECT * FROM jobs WHERE expiry_date > ? ORDER BY id DESC LIMIT 10", [Date.now()]);
    ...
    var count = await poolQuery("SELECT COUNT(*) as num FROM jobs", []);
    res.render('index', {
        cities,
        jobs,
...
router.get('/msgCount', auth, async (req, res)=>{
    const data = await poolQuery("SELECT COUNT(*) as num FROM messagelist WHERE " + req.userData.type + "_id=? AND seen=0 AND sender!=?", [req.userData.id,req.userData.type]);
    res.json({
        count: data[0].num
    });
});
